...
jQuery(document).ready(function(e) {
	...
	jQuery(document).on('click','#scrollTop',function(){
		jQuery('#scrollTop').animate({
			top:10
			},1500,function(){jQuery('#scrollTop').hide().css('top','auto');}
		);
		jQuery('body,html,header').animate({scrollTop:0},1000);	
	});
	
	jQuery(document).on('submit','form[name="personalize_wishes"]', function(e){
		e.preventDefault();
	
		var name = jQuery("#name").val();
		
		if (name.length>50) {
			alert('Name is Over size!'); 
			return false;
		}
		if (name.length==0) {
		...
		//name = escape(name).replaceAll("%","!");
		var url="http://10.10.2.36/2015/index.html?name="+name;
		
		jQuery('#modal').slideDown('slow');
		jQuery('#alert').html(url).attr('href',url);
		jQuery('#close_alert').attr('data-url',url);
		
		return false;
	});
	
	jQuery(document).on('click','#close_alert', function(){
		var url  = jQuery('#close_alert').attr('data-url');
		jQuery('#modal').slideUp('slow');
		if(url){window.location.href = url;}
	});
	
	jQuery('body').wpSuperSnow({
        flakes: ['pic/stars/star1.png','pic/stars/star2.png','pic/stars/star3.png', 'pic/stars/star4.png', 'pic/stars/star5.png'],
        totalFlakes: '35',
        zIndex: '999999',
        maxSize: '77',
        maxDuration: '40',
...
var defaultName="We All";
var uriName = QueryString("name");

if(!uriName){
	uriName=defaultName;
}

finalName = decodeURIComponent(uriName);

jQuery('.username').html(finalName);